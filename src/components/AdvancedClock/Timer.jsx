...
const Timer = () => {
    ...
    return (
        <div ref={containerRef} style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'hidden' }}>
            ...
            {isFinished ? (
                <div className="timer-finish-anim" style={{ textAlign: 'center', marginTop: '2rem' }}>
                    <Bell size={80} color="var(--neon-yellow)" style={{ marginBottom: '1rem' }} />
                    <h2 className="neon-text-yellow" style={{ fontSize: '3rem', marginBottom: '2rem' }}>Time's Up!</h2>
                    <button className="btn-neon" onClick={resetTimer} style={{ fontSize: '1.5rem', padding: '1rem 2rem' }}>Dismiss</button>
                </div>
            ) : (
                <>
                    {/* 3D Viewport */}
                    ...
                    {remainingTime === 0 && !isRunning ? (
                        <div className="stagger-entry glass-panel" style={{ padding: '2rem', display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '2rem' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <input
                                    type="number" min="0" value={inputHours}
                                    onChange={(e) => setInputHours(parseInt(e.target.value) || 0)}
                                    className="neon-input"
                                    style={{ width: '60px', textAlign: 'center', fontSize: '2rem', padding: '0.5rem' }}
                                />
                                <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)', marginTop: '0.5rem' }}>HRS</span>
                            </div>
                            <span style={{ fontSize: '2rem', color: 'white', paddingBottom: '1.5rem' }}>:</span>
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <input
                        ...
                        <div className="stagger-entry glass-panel" style={{ padding: '1rem 2rem', marginBottom: '2rem', textAlign: 'center' }}>
                            <div style={{ fontSize: '3rem', fontFamily: 'monospace', fontWeight: 'bold', color: 'var(--neon-yellow)' }}>
                    ...
                    <div ref={controlsRef} className="stagger-entry" style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
                        {!isRunning && remainingTime === 0 ? (
                            <button className="btn-neon" onClick={startTimer} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1.2rem', padding: '0.8rem 1.5rem' }}>
                                <Play size={24} /> Start Timer
                            </button>
                        ) : (
                            <>
                                <button className="btn-neon" onClick={isRunning ? pauseTimer : startTimer} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    {isRunning ? <Pause size={20} /> : <Play size={20} />}
                                    {isRunning ? 'Pause' : 'Resume'}
                                </button>
                                <button className="btn-neon" onClick={resetTimer} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', borderColor: '#ff0055', color: '#ff0055' }}>
